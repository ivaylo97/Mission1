...
import eu.deltasource.internship.hotelserviceapplication.Support.DateExceptions.InvalidValuesException;
...
public class Booking {
	...
	private void assertAndSetID(String id) {
		if (id == null || id.length() < 10){
			throw new InvalidValuesException("ID has invalid value");
		}
		guestID = id;
	...
	private void assertAndSetDates(LocalDate fromDate, LocalDate toDate) {
		if (fromDate != null && toDate != null) {
			if (fromDate.isBefore(LocalDate.now())) {
				throw new InvalidValuesException(fromDate.toString() + " is before today!");
			}
			if (toDate.isBefore(LocalDate.now())) {
				throw new InvalidValuesException(toDate.toString() + " is before today !");
			}
			if (fromDate.isAfter(toDate)) {
				throw new InvalidValuesException(fromDate.toString() + " is after " + toDate.toString() + " !");
			}
			startDate = fromDate;
			endDate = toDate;
		}

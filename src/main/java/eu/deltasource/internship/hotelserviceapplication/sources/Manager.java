...
public class Manager {
	...
	public int bookRoom(String guestEGN, LocalDate fromDate, LocalDate toDate, int requiredCapacity) {
		...
		if (!listOfRooms.isEmpty()) {
			...
			for (Room room : listOfRooms) {
				roomNumber = room.createBooking(guestEGN, fromDate, toDate, room);
				if (roomNumber >= 0) {
	...
	public void unBookRoom(String guestEGN, LocalDate fromDate, LocalDate toDate) {
		Room room;
		for (int roomsCounter = 0; roomsCounter < managedHotel.getListOfRooms().size(); roomsCounter++) {
			room = managedHotel.getListOfRooms().get(roomsCounter);
